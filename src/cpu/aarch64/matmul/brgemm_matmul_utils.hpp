...
struct brgemm_matmul_conf_utils_t {
    ...
    inline bool use_buffer_b(bool use_heuristic = true) const {
        ...
        // if (bgmmc.M == 1 && bgmmc.wei_tag == format_tag::ab) {
        if (bgmmc.M == 1 && (bgmmc.wei_tag == format_tag::ab || bgmmc.wei_tag == format_tag::ba)) {
            printf("=== 1xK:KxN GEMV - no copying/transposing of B ===\n");
            return false;
        }
